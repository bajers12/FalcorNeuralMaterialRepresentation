...
class RayPairDataset(Dataset):
    """
    Loads pre-generated BRDF training samples from .npz.
    ...
    def __init__(self, path: str):
        ...
        self.wo = self.data["wo"]
        self.y = self.data["y"]
        self.normal = self.data.get("normal", None)

        n = self.uv.shape[0]
        assert self.wi.shape[0] == n and self.wo.shape[0] == n and self.y.shape[0] == n
        assert self.uv.shape[1] == 2 and self.wi.shape[1] == 3 and self.wo.shape[1] == 3 and self.y.shape[1] == 3
        if self.normal is not None:
            assert self.normal.shape[0] == n and self.normal.shape[1] == 3

    def _load(self, path: str) -> Dict[str, torch.Tensor]:
        ext = os.path.splitext(path)[1].lower()
        if ext != ".npz":
            raise ValueError(f"Unsupported file extension: {ext} (only .npz supported in this version)")
        ...
        for k in ("uv", "wi", "wo"):
            if k not in arr:
                raise ValueError(f"Missing key '{k}' in npz")

        # target: accept either y or rgb
        if "y" in arr:
            y_np = arr["y"]
        elif "rgb" in arr:
            y_np = arr["rgb"]
        else:
            raise ValueError("Missing target in npz: expected key 'y' or 'rgb'")

        out: Dict[str, torch.Tensor] = {
            "uv": torch.from_numpy(arr["uv"]).float(),
            "wi": torch.from_numpy(arr["wi"]).float(),
            "wo": torch.from_numpy(arr["wo"]).float(),
            "y": torch.from_numpy(y_np).float(),
        }

        # optional normals
        if "normal" in arr:
            out["normal"] = torch.from_numpy(arr["normal"]).float()
...
def make_optimizer(model: NeuralMaterialModel, cfg: TrainConfig) -> torch.optim.Optimizer:
    ...
    if cfg.train_latent_texture:
        params.append({"params": model.latent.parameters()})
    if cfg.train_decoder:
        params.append({"params": model.decoder.parameters()})
    if not params:
        raise ValueError("Nothing to train: both train_latent_texture and train_decoder are False")
    return torch.optim.Adam(params, lr=cfg.lr, weight_decay=cfg.weight_decay)


def make_scheduler(opt: torch.optim.Optimizer, cfg: TrainConfig):
    """
    Cosine LR decay from cfg.lr to cfg.lr_min over cfg.max_epochs (epoch-stepped).
    """
    def lr_lambda(epoch: int):
        ...
        lr_now = cfg.lr_min + (cfg.lr - cfg.lr_min) * scale
        return lr_now / cfg.lr

    return torch.optim.lr_scheduler.LambdaLR(opt, lr_lambda=lr_lambda)


def maybe_freeze_parts(model: NeuralMaterialModel, cfg: TrainConfig, epoch: int) -> None:
...
def train_one_epoch(
    ...
    for step, batch in enumerate(loader):
        ...
        if (step + 1) % cfg.print_every_steps == 0:
            dt = time.time() - t0
            print(
                f"[train] epoch {epoch:03d} step {step+1:05d} "
                f"loss={running_loss/n_batches:.6f} mae={running_mae/n_batches:.6f} "
                f"lr={opt.param_groups[0]['lr']:.2e} time={dt:.1f}s"
            )

    return {"loss": running_loss / max(1, n_batches), "mae": running_mae / max(1, n_batches)}
...
def validate(model: NeuralMaterialModel, loader: DataLoader, cfg: TrainConfig, epoch: int) -> Dict[str, float]:
    ...
    print(f"[val] epoch {epoch:03d} val_loss={out['val_loss']:.6f} val_mae={out['val_mae']:.6f}")
    return out
...
def main():
    ...
    for epoch in range(cfg.max_epochs):
        maybe_freeze_parts(model, cfg, epoch)

        train_metrics = train_one_epoch(model, train_loader, opt, cfg, epoch)
